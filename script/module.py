...
import time
# 导入操作excel文件函数，如果没有安装，可通过pip install openpyxl安装
...
# 定义浏览器启动函数，本次并没有使用common中定义的browserlauncher函数，读者可自行拓展写
def get_driver(testpage,teststep,testdata):
    #设置浏览器启动函数执行状态，便于后续运行控制
    get_driver=True
    # 判断测试用例是否需要启动浏览器，如果需要
    if testpage=='浏览器':
        if teststep=='Chrome':
            driver=webdriver.Chrome("C:\Program Files (x86)\Google\Chrome\Application\chromedriver.exe")
        else:
            logger.info('未知浏览器类型，请检查测试用例')
        # 启动没有问题后加载测试路径并返回driver 对象
        driver.get(testdata)
        get_driver=driver
    else:
        # 如果用例中的启动参数错误，则写入日志并给予提示
...
def exec_script(driver,testpage,teststep,testdata):
    ...
    try:
        ...
        if testpage=='登录':
            ...
            if teststep == '用户名':
                login.input_users(testdata)

            if teststep == '密码':
                login.input_userpwd(testdata)
